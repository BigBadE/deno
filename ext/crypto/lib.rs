...
pub fn op_crypto_get_random_values(
    ...
    if zero_copy.len() > 65536 {
        return Err(
            custom_error("DomExceptionQuotaExceededError", format!("The ArrayBufferView's byte length ({}) exceeds the number of bytes of entropy available via this API (65536)", zero_copy.len()))
                .into(),
        );
    }
